...
Piracy? RuTracker is a big preservation platform, like Internet Archive. Many authors/musicians/creators publish their artifacts to RuTracker. Also it has many abandoned artifacts that you cannot buy anymore. Also public domain, creative commons artifacts. And piracy. Sorry for that, but I love RuTracker, I respect their activities about culture preservation (as Internet Archive - thank you too). There is no link to RuTracker in this README file.
...
- The VM worker runs beside tdlib's local `telegram-bot-api` server for uploads above the public Bot API limit.
...
The Oracle stack expects an ARM-compatible image OCID for your chosen region. It provisions `VM.Standard.A1.Flex` with 4 OCPUs, 24 GB RAM, and a 200 GB boot volume by default, then builds tdlib `telegram-bot-api` and this bot on the VM.
